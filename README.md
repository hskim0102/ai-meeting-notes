...
| **Dify RAG 자동 생성** | 회의록 DB 저장 성공 시 Dify RAG 에이전트 호출 → 지식 도큐먼트 등록 후 `meeting_rag_docs` 테이블에 document_id 매핑 저장 |
...
POST /api/meetings (신규 생성)
  ...
  inputs: { id, gubun: '1', meetings_note }
        │
        ├─ 성공 → document_id 수신
...
PUT /api/meetings/:id (수정)
  ...
  Dify Workflow API 비동기 호출
  POST {DIFY_API_URL}/workflows/run
  inputs: { id, gubun: '2', document_id: <기존값 or ''>, meetings_note }
        │
        ├─ 성공 → document_id 수신
        │         meeting_rag_docs UPDATE { document_id, status: 'completed' }
        │
...
| 신규 회의 저장 시 RAG 에이전트 호출 | **완료** | Dify Workflow API 직접 호출 (gubun: '1') |
| 회의 수정 시 RAG 에이전트 재호출 | **완료** | 기존 document_id 포함하여 호출 (gubun: '2') |
| Dify 응답 파싱 | **확인 필요** | `body.data.outputs.document_id` 가정 — Dify 워크플로우 출력 변수명 확인 필요 |
...
```json
{
  ...
  "gubun": "1",
  "document_id": "",
  "meetings_note": "{\"id\":123,\"title\":\"...\",\"date\":\"...\", ... }"
}
```

- `gubun`: `'1'` = 신규 생성, `'2'` = 수정
- `document_id`: 수정 시 기존 Dify 도큐먼트 ID (신규 생성 시 빈 문자열)
- `meetings_note`: `formatMeeting()` 결과를 JSON 직렬화한 문자열
