...
| **Dify RAG 자동 생성 (예정)** | 회의록 DB 저장 성공 시 Dify 웹훅 트리거 호출 → RAG 문서 자동 등록 |
...
### Dify 웹훅 트리거 (RAG 자동 생성, 예정)
| Method | Endpoint | 설명 |
|--------|----------|------|
| `POST` | `/api/dify/webhook/rag` | 회의록 저장 후 Dify 웹훅 호출 (트리거 테스트용) |
...
## Dify RAG 자동 생성 연동 (예정)

회의록이 DB에 저장되는 시점에 Dify 에이전트를 자동 호출하여 해당 회의록을 RAG 문서로 등록하는 기능입니다.

### 플로우

```
POST /api/meetings (회의 저장)
        │
        ▼
  MySQL DB 저장 성공
        │
        ▼
  DB 저장 파라미터를 JSON으로 구성
  {
    "meeting_id": 123,
    "title": "...",
    "date": "...",
    ...
    ...
        ...
        ▼
  Dify 웹훅 트리거 호출 (POST)
  DIFY_WEBHOOK_URL + DIFY_WEBHOOK_SECRET
        │
        ▼
  Dify 에이전트 응답 수신 (트리거 성공 여부 확인)
        │
        ▼ (추후 구현)
  Dify RAG 문서 자동 등록
```
...
| 단계 | 상태 | 설명 |
|------|------|------|
| 웹훅 트리거 호출 & 응답 수신 | **구현 예정** | DB 저장 성공 시 Dify 웹훅 POST 전송, 응답 로깅 |
| RAG 문서 자동 등록 | 미구현 | Dify Knowledge API를 통한 문서 업로드 |
...
```env
# Dify 웹훅 트리거
DIFY_WEBHOOK_URL=https://your-dify-instance/webhook/xxxxx
DIFY_WEBHOOK_SECRET=your-webhook-secret   # 선택
```
...
| `DIFY_WEBHOOK_URL` | X | Dify 웹훅 트리거 URL (RAG 자동 생성용) | — |
| `DIFY_WEBHOOK_SECRET` | X | Dify 웹훅 시크릿 (선택) | — |
