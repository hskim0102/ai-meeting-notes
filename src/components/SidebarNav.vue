...
const navItems = [
  ...
  { name: '회의 검색', path: '/search', icon: 'search' },
  { name: '회의실 예약', path: '/rooms', icon: 'rooms' },
]
  ...
  <aside class="w-64 bg-slate-900 text-white flex flex-col shrink-0">
    ...
    <nav class="flex-1 p-4 space-y-1">
      ...
      >
        ...
        <!-- Search -->
        <svg v-if="item.icon === 'search'" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
        </svg>
        <!-- Rooms -->
        <svg v-if="item.icon === 'rooms'" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
