        ...
        <template v-for="(step, idx) in steps" :key="step.num">
          ...
          <div class="flex flex-col items-center relative z-10">
            ...
            <div
              ...
              class="w-10 h-10 rounded-full bg-primary-500 flex items-center justify-center shadow-sm cursor-pointer transition-all hover:bg-primary-600"
            ...
            <!-- 현재 스텝 -->
            <div
              v-else-if="currentStep === step.num"
              class="w-10 h-10 rounded-full border-[3px] border-primary-500 flex items-center justify-center transition-all"
              :class="isDark ? 'bg-slate-800' : 'bg-white'"
            >
              <span class="text-sm font-bold text-primary-500">{{ step.num }}</span>
            </div>
            <!-- 미래 스텝 -->
            <div
              v-else
            ...
            >
              <span class="text-sm font-medium" :class="isDark ? 'text-slate-500' : 'text-slate-400'">{{ step.num }}</span>
            </div>
            <!-- 라벨 -->
            <span
